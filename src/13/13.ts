...
var getAllMultipliersOfGivenBus = (buses: number[][], busIndex: number) : Multiplier[] => {
    var multipliers : Multiplier[] = [];
    for (const i of h.range(0, buses.length)) if (i != busIndex) multipliers.push(getAllowedMultipliers(buses[busIndex], buses[i])[0]);
    return multipliers;
}
...
h.print(h.stringify(getAllMultipliersOfGivenBus(buses2, 0)));